...
The policy blocks branch deletion and non-fast-forward updates; requires a pull request, one approval, last-push approval, stale-review dismissal, and resolved review threads; permits merge commits so promotion ancestry remains auditable; requires `validate-source`, `validate-merge-result`, `promotion-guard`, and `immutable-release-gate`; and has no bypass actor.
