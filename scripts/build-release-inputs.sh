...
jq -n --arg repository 'https://github.com/caddyserver/caddy' --arg source_sha "$CADDY_SOURCE_SHA" --arg wrapper_sha "${GITHUB_SHA:-$(git -C "$ROOT" rev-parse HEAD)}" --arg binary_sha256 "$(sha256sum "$BUILD/caddy" | cut -d' ' -f1)" --arg go_mod_sha256 "$(sha256sum "$BUILD/caddy-source/go.mod" | cut -d' ' -f1)" --arg go_sum_sha256 "$(sha256sum "$BUILD/caddy-source/go.sum" | cut -d' ' -f1)" --arg config_sha256 "$config_sha256" '{schema:"codestra.caddy.binary-build.v2",source_repository:$repository,source_sha:$source_sha,wrapper_sha:$wrapper_sha,binary_sha256:$binary_sha256,go_mod_sha256:$go_mod_sha256,go_sum_sha256:$go_sum_sha256,config_sha256:$config_sha256}' > "$BUILD/caddy-binary-build-attestation.json"
