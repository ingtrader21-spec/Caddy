...
def test_configuration_digest_matches_exact_desired_state(documents):
    candidate, _ = documents
    assert candidate["configuration"]["configuration_sha256"] == "f5226881a0df078b347fe75135a18ee5a371a22ca2c7ac355fc054a533195671"
...
def test_all_six_runtime_start_gates_are_required_and_live_execution_is_not_preapproved(documents):
    ...
    assert len(rows) == 6
    assert sum(row["satisfied"] is True for row in rows) == 1
...
def test_evidence_template_requires_merged_sha_only_at_execution(documents):
    ...
    forged["candidate"]["merged_caddy_source_sha"] = validator.EXPECTED_PR_HEAD
    expect_evidence_failure(forged, "merged source must be populated only after PR #175 merges")
