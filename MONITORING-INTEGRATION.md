...
Keep monitoring backend APIs private. Send application API reads through authenticated Middleware/BFF routes, never browser-held backend credentials. Preserve the approved native Grafana and Superset UI routes with Keycloak OIDC and private upstreams in `config/observability-exposure.v1.json`; OpenBao management additionally retains its source-network gate. These native UI routes do not grant a direct operational write path. Use release-mounted secrets, approved targets/query templates, tenant and campaign scopes, and structured redacted telemetry. Service registration, green CI and successful ingestion are distinct from verified production coverage.
